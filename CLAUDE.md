...
`brew generate-tap-completions` generates shell completions for commands in `cmd/`.
...
# Regenerate shell completions after any cmd_args change
...
`scripts/annotate.sh` special-cases `.fish` completion files: because the generator overwrites
their content, they use `.license` sidecars created with `--force-dot-license` rather than
...
Homebrew tap external commands can ship man pages in `manpages/brew-<command>.1.md`.
The `Homebrew::Completions` module does not currently provide an API for generating man pages
from `cmd_args`, so man page sources would need to be written and updated by hand alongside
`cmd_args` changes. Man pages are not yet implemented for this tap.
...
`brew generate-tap-completions` and committing the result. CI verifies these are current.
