...
**Fix in W8.2:** the check is removed outright, not
replaced. See `01-ast-migration-plan.md` § "On idempotency"
for the full rationale. Briefly: duplicate `xattr -d
com.apple.quarantine PATH` exits 0 with no output when the
xattr is absent (functionally harmless); multiple `caveats`
stanzas concatenate at runtime (visible but harmless);
`cask-extract --no-quarantine` is normally one-shot per
cask, making re-runs rare; the cosmetic value of avoiding
duplicate stanzas in that rare case doesn't justify an AST
predicate's variant-matching complexity (`-d` vs `-dr`,
`args:` keyword array vs splat, nested vs top-level).

If real-world use surfaces duplicate insertion as a genuine
irritant, the simplest possible AST predicate ("any
postflight body calling `system_command "/usr/bin/xattr"` at
all?") can be added later without the full variant
matching.
